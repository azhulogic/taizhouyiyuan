...
@Scope("prototype")
public class RcjcAction extends ActionSupport{
	...
	private List<RcjcTodoList> rcjcWcTodoList;
	...
	public String execute(){
		...
		if(flag != null && "todolist".equals(flag) ){
			//验证传参
		...
		}else if(flag != null && "manager".equals(flag) ){
			
			forwordpage = "manager";
		}

		
		if(jczt==null || "".equals(jczt) || "0".equals(jczt)){
			jczt = "0";
			yyrq1 = DateUtil.getDateBeforeStr(new Date(), 40);
			...
			this.rcjcWcTodoList = seniorTalentRcjcService.selectTodoWcList(seniorTalentRcjc);
		}
		if(jczt.equals("1"))
		{
			seniorTalentRcjc.setJczt(new Integer(jczt));
		...
		/*if(yyrq1==null || "".equals(yyrq1)){
			yyrq1 = DateUtil.getDateBeforeStr(new Date(), 3 );
			Date yyrq1_d = DateUtil.string2Date(yyrq1, "yyyy-MM-dd");// DateUtil.getBeforDate(DateUtil.parseDateString(yyrq1, "yyyy-MM-dd"),7);
			seniorTalentRcjc.setYyrq1(yyrq1_d);
		}else{
	...
	public List<RcjcTodoList> getRcjcWcTodoList() {
		return rcjcWcTodoList;
	}

	public void setRcjcWcTodoList(List<RcjcTodoList> rcjcWcTodoList) {
		this.rcjcWcTodoList = rcjcWcTodoList;
	}
